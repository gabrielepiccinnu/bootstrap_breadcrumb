...
function bootstrap_breadcrumb()
{
    ...
    if ((is_front_page()) && (is_home())) {
        $html .= '<li itemprop="itemListElement" itemscope itemtype="' . $schema_link . '" class="breadcrumb-item active" aria-current="page"><span itemprop="name">' . __('Home', 'gabrielepiccinnu') .  '</span><meta itemprop="position" content="' . $position++ . '" /></li>';
    } elseif (is_front_page()) {
        // static homepage
        $html .= '<li itemscope itemtype="' . $schema_link . '" class="breadcrumb-item active" aria-current="page"><span itemprop="title">' . __('Home', 'gabrielepiccinnu') . '</span></li>';
    } elseif (is_home()) {
        // blog page
        $html .= '<li itemscope itemtype="' . $schema_link . '" class="breadcrumb-item active" aria-current="page"><span itemprop="title">' . __('Blog', 'gabrielepiccinnu') . '</span></li>';
    } else {
        $html .= '<li itemprop="itemListElement" itemscope itemtype="' . $schema_link . '" class="breadcrumb-item"><a href="' . esc_url(home_url('/')) . '" itemprop="item"><span itemprop="name">' . __('Home', 'gabrielepiccinnu') . '</span></a><meta itemprop="position" content="' . $position++ . '" /></li>';
        ...
        if (get_page_by_path('blog')) {
            if (!is_page()) {
                $html .= '<li itemprop="itemListElement" itemscope itemtype="' . $schema_link . '" class="breadcrumb-item"><a href="' . get_permalink(get_page_by_path('blog')) . '" itemprop="item">' . $sep . '<span itemprop="name">' . __('Blog', 'gabrielepiccinnu') . '</span></a><meta itemprop="position" content="' . $position++ . '" /></li>';
            }
        }
        ...
        if (is_category()) {
            ...
            $offercategories = get_the_terms(get_the_ID(), 'offer_category');

            if ($offercategories[0]) {
                $html .= custom_get_category_parents($offercategories[0], 'offer_category');
            }
